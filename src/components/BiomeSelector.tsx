            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {BIOMES.map((biome) => {
                    ...
                    return (
                        ...
                        ${isActive ? `bg-amber-900/20 border-amber-500 shadow-lg shadow-amber-900/20` : ''}
                        ${!isActive && isUnlocked ? 'bg-slate-900 border-slate-800 hover:border-amber-500/30 cursor-pointer' : ''}
                        ...
                        >
                            <div className="flex items-start gap-4">
                                ...
                                <div className={`shrink-0 w-12 h-12 rounded-lg flex items-center justify-center transition-colors
                                    ...
                                    ${!isActive && isUnlocked ? 'bg-slate-950 text-slate-500 group-hover:text-amber-400' : ''}
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex justify-between items-start">
                                        <h3 className={`font-bold text-sm truncate pr-2 ${isActive ? 'text-amber-200' : 'text-slate-200'} ${!isUnlocked ? 'text-slate-600' : ''}`}>
                                            {biome.name}
                                        </h3>
                                        {isActive && <span className="text-[10px] font-bold bg-amber-500 text-white px-1.5 py-0.5 rounded uppercase tracking-wider">Active</span>}
                                        {!isUnlocked && <Lock className="w-4 h-4 text-slate-700 shrink-0" />}
                                    </div>
                                    <p className={`text-xs mt-1 leading-snug ${isActive ? 'text-amber-300/70' : 'text-slate-500'} ${!isUnlocked ? 'text-slate-700' : ''}`}>
                                        {biome.description}
                                    </p>
                                </div>
                            </div>
